...
const Patrouille = () => {
  ...
  const refreshGMR = async () => {
    ...
    if (quartier) {
      await axios.get(`https://patrouilleverte.azurewebsites.net/fetchgmr${quartier}`);
      alert('GMR updated');
    } else {
  ...
  const refreshEAU = async () => {
    ...
    alert('Refreshing, please wait...');
    if (quartier) {
      await axios.get(`https://patrouilleverte.azurewebsites.net/fetcheau${quartier}`);
      alert('EAU updated');
    } else {
      alert('Quartier is not set');
    }
  ...
  const refreshFORET = async () => {
    ...
    alert('Refreshing, please wait...');
    if (quartier) {
      await axios.get(`https://patrouilleverte.azurewebsites.net/fetchforet${quartier}`);
      alert('FORET updated');
    } else {
      alert('Quartier is not set');
    }
    ...
    <ParallaxScrollView
      ...
      <ThemedView style={styles.stepContainer}>
        <Collapsible title="GMR" collapsed={!showGMR} onToggle={() => setShowGMR(!showGMR)}>
          <DynamicForm apiEndpoint={`https://patrouilleverte.azurewebsites.net/gmr${quartier}`}/>
      ...
      <ThemedView style={styles.stepContainer}>
          <Collapsible title="EAU" collapsed={!showEAU} onToggle={() => setShowEAU(!showEAU)}>
            <DynamicForm apiEndpoint={`https://patrouilleverte.azurewebsites.net/eau${quartier}`}/>
      ...
      <ThemedView style={styles.stepContainer}>
          <Collapsible title="FORET" collapsed={!showFORET} onToggle={() => setShowFORET(!showFORET)}>
            <DynamicForm apiEndpoint={`https://patrouilleverte.azurewebsites.net/foret${quartier}`}/>
